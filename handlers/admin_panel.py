...
def register_admin_panel(bot):
    ...
    def vip_users(c):  
        ...
        cur = conn.cursor()  
        cur.execute("""  
            SELECT u.first_name, u.username, c.user_id, c.balance  
            FROM credits c  
    ...
    def admin_input(m):
        ...
        try:
            ...
            # ==== CREDITS UNLIMITED ====  
                ...
                cur = conn.cursor()
                cur.execute(
                    "UPDATE credits SET balance = -1 WHERE user_id = ?",
                    (uid,)
                )
                conn.commit()
                conn.close()
            
                bot.send_message(m.chat.id, "♾ User set to Unlimited Credits")
                ADMIN_STATES.pop(m.from_user.id, None)
                return
            # ==== CREDITS CHECK ====
            ...
            elif action == "code:count":  
                ADMIN_STATES[m.from_user.id] = {  
                    "action": "code:credits",  
                    "count": state["count"],  
                    "credits": int(m.text)  
                }  
                bot.send_message(m.chat.id, "Send max uses:")  
                return  
  
            elif action == "code:credits":  
                codes = [  
                    create_code(state["credits"], int(m.text))  
...
# ===== BUY PACKAGE ADD =====
                ...
                bot.send_message(
                    m.chat.id,
                    "✅ Buy package created successfully"
                )
                return
