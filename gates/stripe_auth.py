...
class StripeProcessor:
    ...
    def _execute_user_registration(self):
        ...
        registration_nonce = self._extract_nonce_value(initial_response.text,
    ...
    def _retrieve_payment_nonce(self):
        response = self.session_manager.get(ADD_PAYMENT_PAGE, headers=self._create_request_headers())
        return self._extract_nonce_value(response.text, r'"createAndConfirmSetupIntentNonce":"(.*?)"')

    def _create_payment_method(self):
        countries = ["IQ", "US", "UK", "DE", "FR", "CA"]  # اختار عشوائي
...
# دالة الفحص الرئيسية
def check(card):
    if not GATE_FUNCTIONS:
        GATE_FUNCTIONS.append(str1)  # تأكد من وجود دالة واحدة على الأقل

    func = random.choice(GATE_FUNCTIONS)  # اختيار دالة عشوائية
    result = func(card)
...
# ======================================================
# دالة تحميل ملفات بوابة جديدة
def load_gate_file(file_path):
    module_name = os.path.basename(file_path).replace(".py", "")
    spec = importlib.util.spec_from_file_location(module_name, file_path)
    if spec is None:
        print(f"⚠️ لا يمكن تحميل الملف: {file_path}")
        return
    ...
    try:
        ...
        print(f"⚠️ خطأ أثناء تنفيذ الملف: {e}")
        return

    # إضافة أي دالة تبدأ بـ "str" تلقائيًا
    for attr_name in dir(module):
        attr = getattr(module, attr_name)
        if callable(attr) and attr_name.startswith("str"):
            GATE_FUNCTIONS.append(attr)
            print(f"✅ تمت إضافة الدالة {attr_name} من الملف {file_path}")